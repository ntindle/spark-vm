...
if [ -n "$STAGED" ]; then
    LEAK="$(echo "$STAGED" | xargs grep -nEi '(sk-[A-Za-z0-9_-]{16,}|ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|AKIA[0-9A-Z]{16}|LLM_[0-9]{3,}_[A-Za-z0-9-]{16,}|-----BEGIN (RSA |EC |OPENSSH )?PRIVATE KEY-----|xox[bap]-)' 2>/dev/null || true)"
    if [ -n "$LEAK" ]; then
