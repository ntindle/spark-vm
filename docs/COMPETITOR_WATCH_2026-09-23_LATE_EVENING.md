...
- **boat.dev** — rate card unchanged: small **$0.018/h**, default
  **$0.036/h**, large **$0.072/h**, xlarge **$0.200/h** (xlarge needs a
  ...
  first payment (**VERIFIED**: https://docs.boat.dev/pricing, ~08:03).
...
- **DigitalOcean Managed Agents** — still **Public Preview**; hero
  ...
  https://docs.digitalocean.com/products/managed-agents/, ~08:04).
...
VM. Not a hosted-always-on SKU (an anti-always-on billing model), but
(**INFERRED** lane judgment) a same-lane **reference architecture**
for serverless agent-sandbox control planes — worth keeping on the
watch list as design color, not a corpus entry.

### 3c. Herdr × Vercel Sandbox plugin — same-lane validation of one-agent-one-machine

Newly surfaced (Vercel changelog, **VERIFIED** — fetched this pass
~07:55–08:05 CDT; the changelog entry itself is dated 2026-09-22): the
...
credential-never-leaves-sandbox, and git-patch apply. This is
(**INFERRED** lane judgment) a **same-lane** datapoint validating the
"one agent, one persistent machine" model spark-vm's hosted product is
betting on — filed as watch color, not a new tracked provider (it is
Vercel Sandbox usage, not a new sandbox SKU).
...
- **Google PM's "Always On Memory Agent"** (open source, ADK + Gemini
  ...
  (**INFERRED** lane judgment) adjacent.
- **Andon Labs Pion research preview** (Sept 14 — **VERIFIED** on the
  ...
  experiments. Provenance promotion this pass: snippet-only in the
  evening pass, own-page confirmed now. (**INFERRED** lane judgment)
  Adjacent — a persistent-agent *ops platform* with its
...
- **Automaid "always-on AI operations hub"** (2026-09-21 — still
  ...
  **THIRD-PARTY**, plus aggregator digest snippet-only). (**INFERRED**
  lane judgment) Adjacent — a
...
- **Huawei Cloud "Open Agentic Cloud"** (announced Sept 18/19 — still
  **THIRD-PARTY** / snippet-only via syndicated rollout copy):
  ...
  Context Memory Storage, Industry AI Foundry zones. (**INFERRED** lane
  judgment) Adjacent —
...
the canonical corpus. Note on the surveyor recommendation: the ask to
"add DO Managed Agents to the standing watch list" is retired — DO is
already a tracked provider and C26 is the open watch item; the
recommendation folds into the C26 pricing re-check above. Next pass's
ask: routine tracked-set re-reads;
