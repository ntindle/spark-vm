...
## Watch update — 2026-09-26 (early morning): C26 snapshot-rate + active-CPU conflicts resolved; C48 stale-ask closed; fast movers NO-CHANGE

Full pass record in `docs/COMPETITOR_WATCH_2026-09-26_EARLY_MORNING.md`
(survey window 2026-09-26 ~03:58–04:15 CDT; captures in
`agent_notes/surveyor-a-20260926-0354.md` and
`agent_notes/surveyor-b-20260926-0354.md`).

- **C26 — DigitalOcean Managed Agents pricing conflicts RESOLVED
  (in-lane, VENDOR-VERIFIED).** A second vendor-owned dollar-pricing
  surface found and read in full this run:
  `digitalocean.com/pricing/harness-runtime` — CPU **$0.044/vCPU-hour**
  (actual CPU consumed), memory **$0.0095/GB-hour** (peak memory
  used), session storage (volumes) **$0.05/GiB-month**, snapshots and
  checkpoints **$0.05/GiB-month**, custom sandbox templates (BYOT)
  **$0.05/GiB-month**, public internet egress **$0.01/GiB**; shape
  table at full allocation XSmall (1 vCPU/1 GB) $0.0535/hr → XLarge
  (16/32) $1.008/hr; Harness Runtime requires a positive prepaid
  account balance; launch-blog pricing example (VERIFIED full read):
  2 vCPUs @ 25% avg + 4 GB peak for an hour = **$0.060** vs $0.126
  full-allocation. (a) **Snapshot-rate conflict resolved 2:1 for
  $0.05/GiB-month** — the docs pricing subpage and the new pricing
  page both print $0.05; only the 2026-09-22 investor-relations launch
  page prints $0.005 (VENDOR-VERIFIED, re-read this run — no
  correction, no new date), treated as a release-text typo
  (INFERRED); the pricing page is the billing surface of record; the
  2026-09-25 midday blast-radius scope stands (compute and memory
  agree exactly across surfaces; only Snapshots and Checkpoints
  disagree). (b) **Active-CPU conflict narrowed toward the docs
  footnote** — the pricing page states "Active CPU billing is coming
  soon. Until then, you will be billed at 25% of the vCPUs allocated
  to your sandbox"; the IR page's live framing is the oddity. The
  product docs page (`docs.digitalocean.com/products/managed-agents/`,
  "Last verified 21 Sep 2026") still carries no dollar pricing —
  **VERIFIED absent** — but the dollar-pricing ask is answered by the
  new surface, so the "Details pricing subpage" framing is retired.
  **C26 moves to CLOSED-resolved.** Remaining vendor-side corrections
  (the IR page's $0.005 and live-active-CPU framing) are watch-only.
  The DO row's price-signal cell above carries the dated fold note.
- **C48 — Prime Intellect Prime Sandboxes: stale BACKLOG ask closed.**
  The corpus row already records 2026-09-25 afternoon full-page
  vendor reads; this pass re-read the vendor docs live and the filed
  figures hold verbatim — CPU $0.02/vCPU-hr, memory $0.0125/GiB-hr,
  disk $0.0002/GiB-hr, "valid through December 22, 2026"; CPU-only
  access today, GPU microVMs / snapshots / forking / shared
  persistent workspaces on roadmap. **No drift; no corpus change.**
- **"Boat DELTA" branding retired from future citations.** The
  2026-09-25 post-pre-midnight pass's vendor-primary pricing facts
  (`docs.boat.dev/pricing`) stand as facts in the tracked-set Boat
  row; but boat.dev's homepage (full read this run) and three web
  searches found no DELTA-branded announcement, product, tier, or
  changelog entry — the *DELTA as a product announcement* framing is
  unverifiable. Verbatim homepage facts captured for future passes
  (see the watch doc).
- **Fast movers — 4/4 VENDOR-VERIFIED NO-CHANGE.** Daytona changelog
  (newest still SEP 26 V0.218.0 / SEP 25 V0.217.0, character-identical
  to the 02:55 baseline); Docker release notes (newest dated heading
  still 2026-09-22 v0.45.1); Microsandbox releases (newest still
  v0.7.3 #1646); Vercel changelog (newest entries still 25 Sep — no
  26-Sep entries; Drives still public beta, GA watch NO-CHANGE).
  Pricing sweep VENDOR-VERIFIED NO-CHANGE (E2B, Modal — sandbox tier
  now measured ≈3× standard to full precision — 3.01x CPU
  ($0.00003942 vs $0.0000131/core-sec), 3.00x memory; TermSquad;
  Fly.io Sprites; Northflank; Cloudflare
  Sandbox SDK). AgentComputer pricing UNVERIFIED this run (local
  worker transport failure — no NO-CHANGE claim). 1 upstream 404 on
  a mistyped Cloudflare path (recovered); 1 local transport failure;
  zero other fetch failures.
- **Carried:** C37 (Freestyle Pro fee VERIFIED absent again, ~04:00
  CDT), C57 (Baponi), C58 (Leap0) — C57/C58 THIRD-PARTY-only, not
  re-surveyed this pass (honest record: this pass's second surveyor
  carried a mislabeled brief — "C57 Boat DELTA", "C58 DO
