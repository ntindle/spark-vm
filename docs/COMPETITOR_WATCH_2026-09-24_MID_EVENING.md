...
- **DigitalOcean Managed Agents — pricing re-VERIFIED (press release);
  ...
  subpage.) The vendor's own release prints snapshots at
  $0.005/GiB-month while the docs say $0.05/GiB-month — a live 10x
  discrepancy this run, not a settled call; the carried pricing-subpage
  re-fetch must resolve it.
