...
- `X-Spark-Approval-Pending: <aid>[, <aid>...]` — a grant request for
  this request's `(credential, host, method)` tuple is filed (or already
  pending). `<aid>` is the approval id (16 random hex chars). Park the
  task and re-issue the gated request to poll.
- `X-Spark-Approval-Decision: <state>:<aid>[, ...]` — a terminal
  ...
  - `expired:<aid>` — the pending approval the client was waiting on
    ...
    item and filed a replacement, so the client keeps waiting on the
    new id. (If no fresh pending id accompanies it, the replacement
    filing was suppressed — the per-credential flood cap or the 60s
    filing rate limit is active — so keep polling; the replacement
    files once the window clears.)
...
4. `denied:<aid>` → report `human-denied`, stop refiling. `approved:<aid>`
   ...
   filing window clears. A client MAY also stop after repeated
